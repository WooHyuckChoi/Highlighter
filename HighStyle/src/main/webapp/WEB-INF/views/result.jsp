...
<!-- 게시판 타이틀 -->
      ...
      <div class="page-header">
        <div class="row-fluid">
          <div class="span12" style="color:#303030; font-size:30px;">
            <img src="./resources/img/group.png">&nbsp;&nbsp;과외 검색(여긴 result)
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- 검색된 과외 목록 -->
 	...
 	<div id="An_extra_check" style="width:700px;  float:left">
 		<table>
 			<tr style="border-bottom:2px solid #b0b0b0;">
 				<th height="50">강사 명</th>
 				<th>학력</th>
 				<th>지역</th>
 				<th>과목</th>
 				<th>금액(시간당)</th>
 			...
 			<tr style="text-align:center; border-bottom:1px solid #b0b0b0;">
 				<td width="100" height="100" ><a href="${list.user_id}">${list.user_name}</a></td>
 				<td width="200">${list.college}<br/>${list.college_dept}</td>
 				<td width="200">${list.class_poss_area}</td>
 				<td width="50">
				...
				<c:if test="${list.subject eq '03'}">
 				...
 				<td  width="100">${list.tuit_fees}원/1시간<br/><a href="#" class="btn">자세히</a></td>
 				</tr>
 			</c:forEach>
 			</c:if>
 		...
 		<div class="pagination" style="float:center">
 			<ul>
 				<c:if test="${pageMaker.prev}">
 					<li><a href="privateSearch${pageMaker.makeSearchQuery(pageMaker.startPage - 1)}">&laquo;</a></li>
 				</c:if>
 				<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
 					<li
 						<c:out value="${pageMaker.cri.page == idx? 'class =active':'' }"/>>
 						<a href="privateSearch${pageMaker.makeSearchQuery(idx)}">${idx}</a>
 					</li>
 				</c:forEach>
 				<c:if test="${pageMaker.next && pageMaker.endPage > 0 }">
 					<li><a href="privateSearch${pageMaker.makeSearchQuery(pageMaker.endPage + 1)}">&raquo;</a></li>
 				</c:if>
  			</ul>
 		</div>
 	</div>
 	...
 	<div id="Private_search" style="width:250px; border:1px solid #b0b0b0; float:right; padding-top:20px; padding-left:20px; position:relative;">
 		 <div style="border-bottom:2px solid orange; text-align:center; font-size:20px; margin-right:20px; margin-bottom:20px;">과외 검색</div>
 		 <div class="formcol">
 			 <!-- 기본 검색  -->
 			 <!-- 온/오프라인 여부 -->
             ...
             <!-- 과외 시간대 선택(시간표) -->
             <label for="ext_timetable">과외 시간대 선택</label>
             <div id="ext_timetable">
            	<ul class="nav nav-pills">
         		 <li class="active" style=" background-color:white"><a href="#" onclick="layer_open('layer2');" style="background-color:white;"><img src="./resources/img/calendar-with-a-clock-time-tools.png"></a></li>
            	</ul>
             </div>
            ...
            <div class="locationView">   
                <div id="tutoringLocation" class="insertMargin1 banner" style="margin:0; width: 230px;">수업가능지역</div>
                <div id="locationSelect1" class="insertMargin2">가능 지역1
                  ...
                  <div id="locationSelect2" class="insertMargin2">가능 지역2
		   		...
		   		<div>
           			 ...
           			 <input type="text" name="college" id="college" tabindex="3" value="${cri.college}" required>
         		</div>
         		
         		<!-- 시간당 수업료 -->
         		
		   		<div id="houlyWage" class="insertMargin1" style="margin:0">시간당 수업료</div>
		   		<div id="houlyWageInput">
		  	 	...
		  	 	<!-- <input type="text" name="str_tuit_fees" id="str_tuit_fees" />원  -->
		   		<!-- ~ <input type="text" name="end_tuit_fees" id="end_tuit_fees" />원 -->
		   		(최소 입력 단위:10,000)<br />수업료 금액 안에는 서비스 이용 수수료가 포함되어 있습니다.</div><br/>
        		
        		<hr/>
            	<!-- 과외 대상 -->
            	<div>
            	<label for="ext_obj">과외 대상</label>
            	<select id="ext_obj" name="ext_obj">
          		...
          		<!-- 과외 경력 -->
            		<div>
          		<label for="ext_exp">과외 경력 여부</label>
          		...
          		<!-- 자격증 및 경력 순 -->
          		<div>
          		<label for="carrer">자격증 및 경력 여부</label>
          		<div class="onoffswitch">
				...
				<div class="con_fil">
            	 ...
            	 <label for="ext_way">과외 형태</label>
          		...
          		<!-- 수업 레벨 -->
          		<div>
            	 <label for="class_lev">수업 레벨</label>
             ...
             <!-- 검색 버튼 -->
             <div id="search" style="width:100px; float:left; margin-left:50px;">
            	<button id = "SearchBtn">검색</button>
             </div>
             <!-- 상세 검색 버튼 -->
             <div style="width:30px; height:30px; float:right; padding-top:10px; padding-left:10px; padding-right:10px;"><a href=""><img src="./resources/img/add-to-search-symbol.png"></a></div>
          </div>
 	</div>
</div>
